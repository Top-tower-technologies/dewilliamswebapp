...
const NewBookingPage = () => {
    ...
    const fetchAvailableRooms = async () => {
        ...
        try {
            ...
            const response = await axiosInstance.post(
                `/guest/find-stay/apartment_service/anonymous`,
                requestBody
            );
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await axiosInstance.post(
                `/staff/reservations/apartment_service/${selectedRoom.id}/new?pay_now=true`,
                bookingData,
                {
            ...
            <MainLayout
                navigation={
                    <div className='flex justify-center gap-x-3 items-center backdrop-blur-sm bg-white/70 rounded-full px-6 py-2 shadow-sm border'>
                        ...
                        <p className='text-[20px] font-[400] text-gray-500'>Apartment</p>
                        <ChevronRight className="text-gray-300" size={20} />
                        <p className='text-[20px] font-[400] text-yellow-600'>New Apartment</p>
                    </div>
                }
                buttonText=""
                buttonVisible={true}
                ...
                <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
                    ...
                    <Card className="lg:col-span-2 backdrop-blur-sm bg-white/80 shadow-xl">
                        <CardHeader className='flex flex-row justify-between items-center bg-gray-50 rounded-t-lg'>
                            <CardTitle className="text-2xl font-bold text-gray-800">Apartment Details</CardTitle>
                                ...
                                <div className="p-4">
                                    {!bookingData.check_in || !bookingData.check_out ? (
                                        <div className="text-center py-8 text-gray-500">
                                            <p>Please select check-in and check-out dates to see available apartments</p>
                                        </div>
                                    ) : roomsLoading ? (
                                        <div className="text-center py-8 text-gray-500">
                                            <p>Searching for available apartments...</p>
                                        </div>
                                    ) : rooms.length === 0 ? (
                                        <div className="text-center py-8 text-gray-500">
                                            <p>No apartment available for the selected dates and guest count</p>
                                            <p className="text-sm mt-2">Try different dates or adjust the number of guests</p>
                                        </div>
                                    ) : (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                <label className="block text-sm font-medium text-gray-600 mb-2">Available Apartments *</label>
                                            ...
                                            {selectedRoom && (
                                                <div className="bg-amber-50 p-3 rounded-lg">
                                                    <p className="text-sm font-medium text-gray-700">Selected Apartment:</p>
                                                    <p className="text-amber-800 font-semibold">{selectedRoom.category.name}</p>
                                                    <p className="text-gray-600 text-sm">Apartment #{selectedRoom.inventory_id}</p>
                    ...
                    <Card className="backdrop-blur-sm bg-white/80 shadow-xl sticky top-6">
                        ...
                        <CardContent className="space-y-4 p-6">
                            ...
                            {selectedRoom ? (
                                <div className="space-y-3">
                                    <h4 className="font-semibold text-gray-700 border-b pb-1">Apartment & Pricing</h4>
                                    <div className="bg-amber-50 p-3 rounded-lg">
                                        <p className="text-sm font-medium text-gray-700">Selected Apartment:</p>
                                        <p className="text-amber-800 font-semibold">{selectedRoom.category.name}</p>
                                        <p className="text-gray-600 text-sm">Apartment #{selectedRoom.inventory_id}</p>
                                ...
                                <div className="text-center py-8 text-gray-500">
                                    <p>Select a apartment to see pricing</p>
                                </div>
                            )}
